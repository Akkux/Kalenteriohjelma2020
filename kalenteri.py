...
#käyttäjän syötteen mukaan.
    ...
    x = input("""Luo ja muokkaa tapahtumia ('L'), poista tapahtumia ('P')
tai hae tapahtumatietoja ('H'): """)
...
def lisääMuistutuksia(h, vuosi, kuukausi, päivä, tapahtuma):
#Metodi antaa käyttäjän lisätä muistutuksia jo olemassa olevaan tapahtumaan
#tai kirjoittaa tapahtuman muistutukset kokonaan uudestaan.
    x = (input("""Lisää muistutuksia olemassaolevaan tapahtumaan '"""+tapahtuma+"""' ('L')
tai kirjoita kokonaan uudet muistutukset ('K'): """))
    if x == "L":
        print("Lisää muistutuksia tapahtumaan '"+tapahtuma+"'.")
        #Kerätään käyttäjältä uusia muistutuksia.
        muistutus = input("Syötä muistutus/muistutukset pilkuilla erotettuina: ")
        lista = h[vuosi][kuukausi][päivä][tapahtuma]
        #Lisätään uudet muistutukset listaan, joka sisältää tapahtuman muistutukset.
        for item in muistutus.split(","):
            lista.append(item)
        ...
        print("Tallennettu.")
        valinta()
    #Jos käyttäjä haluaa korvata muistutukset uusilla, siirrytään takaisin lisääTapahtumia-metodiin.
    elif x == "K":
        pass
    #Jos syöte on jotain muuta kuin sallitut kirjaimet, aloitetaan metodin suoritus uudestaan.
        ...
        lisääMuistutuksia(h, vuosi, kuukausi, päivä, tapahtuma)
...
#Metodi tallentaa muutokset JSON-tiedostoon.
    ...
    #Jos syötetty tapahtuma on jo olemassa kyseisessä päivässä, siirrytään metodiin "lisääMuistutuksia"
    ...
    #Käyttäjä voi lisätä tapahtumaan haluamansa määrän muistutuksia. Syötteen voi myös jättää tyhjäksi.
    muistutukset = input("""Syötä tapahtumaan liittyvä muistutus/muistutukset pilkuilla erotettuina:
""")
    muistutuslista = []
    #Muistutukset kerätään muistutuslistaan ja asetetaan moniulotteiseen hajautustauluun avaimen
    #h[vuosi][kuukausi][päivä][tapahtuma] arvoksi.
    for item in muistutukset.split(","):
        muistutuslista.append(item)
    ...
    if vuosi in h and kuukausi in h[vuosi] and päivä in h[vuosi][kuukausi]:
        h[vuosi][kuukausi][päivä][tapahtuma] = muistutuslista
    ...
    #päivä-hajautustauluja, joissa on tapahtuma-avaimia, joissa on muistuksia listamuodossa.
    else:
        h[vuosi] = {
            kuukausi: {
                päivä: {
...
#ja se tulostaa ruudulle senhetkisen kuukauden tiedot.
    ...
    #ja siirrytään takaisin valinta-metodiin.
    tulostaTiedot(vuosi, kuukausi)
